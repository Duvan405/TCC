...
def clear_arrays(P:np.array,F:np.array,NTU:np.array)->tuple[np.array,np.array,np.array]:
    dF = np.gradient(F, P)              
    d2F = np.gradient(dF, P)            
    id = np.where(np.diff(np.sign(d2F)) != 0)[0]
    try:
        try:
            P = P[:id[0]]
            F = F[:id[0]]
...
def caso_1A(N_p:int,N_r:int,R:float,NTU:np.array)->tuple[float,float]:
    N = N_p * N_r
    rho = np.exp(-NTU/N)
    lmbda = R * N_r * (1 - rho)
    R_r = N_r * R
    ...
    F = chi / NTU
    return F,P
def data_1A(N_p:int,N_r:int,R_list:list,NTU:np.array)->dict:
    dic = {}
    for R in R_list:
        F,P = caso_1A(N_p,N_r,R,NTU)
        P,F,NTU = clear_arrays(P,F,NTU)
        dic[R] = {'F':F,'P':P,'NTU':NTU}
    return dic


def caso_1B1C(caso:str, R:float, NTU:float, N_p:int, N_r:int)->tuple[float,float]:
    N = N_p * N_r
    rho = np.exp(-NTU / N)
    ...
    F = chi / NTU
    return F, P
def data_1BC(N_p:int,N_r:int,caso:str,R_list:list,NTU:np.array)->dict:
    dic = {}
    for R in R_list:
        P = np.zeros(len(NTU))
        F = np.zeros(len(NTU))
...
def caso_2A(N_p:int,N_r:int,R:float,NTU:np.array)->tuple[np.array,np.array]:
    N = N_p * N_r
    rho = np.exp(-NTU / N)
    lmbda = R * N_r * (1 - rho)
    R_r = N_r * R
    ...
    F = chi / NTU
    return F,P
def data_2A(N_p:int,N_r:int,R_list:list,NTU:np.array)->dict:
    dic = {}
    for R in R_list:
        F,P = caso_2A(N_p,N_r,R,NTU)
        P,F,NTU = clear_arrays(P,F,NTU)
        dic[R] = {'F':F,'P':P,'NTU':NTU}
    return dic
...
def caso_2BC(N_p:int,N_r:int,R:float,caso:str,NTU:np.array)->tuple[np.array,np.array]:
    P= np.zeros(len(NTU))
    F= np.zeros(len(NTU))
    for i,NUT in  enumerate(NTU):
        N = N_p * N_r
...
def data_2BC(N_p:int,N_r:int,caso:str,R_list:list,NTU:np.array)->dict:
    dic= {}
    for R in R_list:
        F,P = caso_2BC(N_p,N_r,R,caso,NTU)
        P,F,NTU = clear_arrays(P,F,NTU)
        dic[R] = {'F':F,'P':P,'NTU':NTU}
    return dic


def caso_3ABC(caso:str, R:float, NTU:float, N_p:int, N_r:int)->tuple[float,float]:
    N = N_p * N_r
    rho = exp(-NTU / N)
    lambda_ = R * N_r * (1 - rho)
...
def data_3ABC( N_p:int, N_r:int,caso:str,R_list:list,NTU:np.array)->dict:
...
def caso_4BC(caso:str, R:float, NTU:np.array, N_p:int, N_r:int)->tuple[float,float]:
    N = N_p * N_r
    rho:float = exp(-NTU / N)
    ...
    return F, P
def data_4BC(N_p:int,N_r:int,caso:str,R_list:list,NTU:np.array)->dict:
    dic={}
    for R in R_list:
        F = np.zeros(len(NTU))
        P = np.zeros(len(NTU))
...
def caso_4A(R:float,NTU:float,N_p:int,N_r:int)->tuple[float,float]:
    # Cálculos iniciales
    N = N_p * N_r
    rho = exp(-NTU / N)
    lambda_ = R * N_r * (1 - rho)
...
def data_4A(N_p:int,N_r:int,R_list:list,NTU:np.array)->dict:
    dic = {}
    for R in R_list:
        F = np.zeros(len(NTU))
        P = np.zeros(len(NTU))
...
def data_NRTC(dic:dict,r:float)->dict:
    dic_N= {'CQ':{},'CF':{}}
    for C in dic['CQ']:
        epi = dic['CQ'][C]['EPI'] 
        ...
        NRTC = 1-Ys
        dic_N['CQ'][C] = {'HERN':NRTC,'Ys':Ys,'NTU':dic['CQ'][C]['NTU']}
    for C in dic['CF']:
        epi = dic['CF'][C]['EPI'] 
        termo1 = C * np.log(1 - epi * (1 - 1/r))
        termo2 = np.log(1 - C * epi * (1 - r))
        ...
        Ys = 1 - (numerador / denominador)
        NRTC = 1-Ys
        dic_N['CF'][C] = {'HERN':NRTC,'Ys':Ys,'NTU':dic['CF'][C]['NTU']}
    return dic_N


def data_EPI(dic:dict)->dict:
    dic_E= {'CQ':{},'CF':{}}
    for R in dic:
        if R <= 1:
            C=R
            dic_E['CF'][C]={'EPI':dic[R]['P'],'NTU':dic[R]['NTU']}
        else:
            C=round(1/R,2)
            dic_E['CQ'][C]={'EPI':dic[R]['P']/C,'NTU':dic[R]['NTU']}
    return dic_E
